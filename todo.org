* TODO Reverse-i search
- [ ] Do a bit of refactoring.
1. tty.js should use a map to deal with the different possible inputs.
2. Utility functions for managing the cursor position should be in cursor.js
3. Make a clearer boundary between cursorPosition (index) and the actual term emulator cursor
   In other words, keep data and display separate.
